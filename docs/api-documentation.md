...
This guide covers the API documentation practices and standards used in CodeCircle, including Swagger/OpenAPI integration, authentication patterns, and RESTful API design principles.
...
The CodeCircle API is documented using OpenAPI 3.0 specifications with automatic generation from NestJS decorators and DTOs.
...
Note: Swagger documentation is only available in development and staging environments. In production, the endpoint is disabled for security.
...
- **API Global Prefix**: /api (configurable via API_PREFIX environment variable)
...
Most endpoints require authentication using the JwtAuthGuard and role-based guards:
